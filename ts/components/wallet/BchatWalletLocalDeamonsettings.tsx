...
{
    const[localDeamonHost,setLocalDeamonHost]=useState('');
    ...
    async function validationForDeamon() {
        ...
        if (confirmation && confirmation.status === 'OK') {
            ...
            setTestNotify(`Height:${confirmation.block_header.height},IP:${localDeamonHost}`)
            return
        }
        setLocalDeamonHost("")
        setLocalDeamonPort('')
...
return<>
 <div className={classNames("wallet-settings-nodeSetting-remoteContentBox-content-hidden-Box")} style={{display: 'block'}}>
                <Flex container={true} justifyContent="space-between">
                    <article className="wallet-settings-nodeSetting-remoteContentBox">
                        ...
                        <input value={localDeamonHost} placeholder="Enter your MainNet IP Address" className="wallet-settings-nodeSetting-remoteContentBox-inputBox" onChange={(e: any) => setLocalDeamonHost(e.target.value)} />
                    ...
                    <article className="wallet-settings-nodeSetting-remoteContentBox">
                        ...
                        <input value={localDeamonPort} className="wallet-settings-nodeSetting-remoteContentBox-inputBox" onChange={(e: any) => !isNaN(e.target.value) && setLocalDeamonPort(e.target.value)} placeholder='please enter your port ' />
                ...
                <div className="wallet-settings-nodeSetting-FlexBox wallet-settings-nodeSetting-remoteContentBox-btnBox">
                    ...
                    <div>
                        <BchatButton buttonColor={BchatButtonColor.Green} text={window.i18n("save")} onClick={() =>Object.keys(verifyDeamon).length === 0 && addDeamonNet()} disabled={Object.keys(verifyDeamon).length === 0 ?true:false} />
                    </div>
