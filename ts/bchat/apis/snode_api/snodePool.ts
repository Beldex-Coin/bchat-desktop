...
export async function TEST_fetchFromSeedWithRetriesAndWriteToDb() {
  ...
  if (!seedNodes || !seedNodes.length) {
    window?.log?.error(
      'BchatSnodeAPI:::fetchFromSeedWithRetriesAndWriteToDb - getSeedNodeList has not been loaded yet'
    );
  ...
  try {
    ...
    window?.log?.error(
      'BchatSnodeAPI:::fetchFromSeedWithRetriesAndWriteToDb - Failed to fetch snode poll from seed node with retries. Error:',
      e
    );
  }
}
